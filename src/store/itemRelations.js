    ...
    for (var i = 0; i < groups.length; i++) {
      ...
      if (group.attributes.length < 100) {
        ...
        for (var j = 0; j < group.attributes.length; j++) {
          ...
          if (attr.relations && (!attr.valid || attr.valid.length === 0 || attr.options.some(opt => opt.name === 'standard_attribute' && opt.value === 'true')) && attr.relations.find(relId => relId === relationId)) {
