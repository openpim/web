<template>
  <div>
    <v-text-field v-if="channel" v-model="channel.config.apiToken" :readonly="readonly" label="API token" required type="password"></v-text-field>
...
export default {
  ...
  setup (props, { root }) {
    ...
    watch(() => props.channel, (chan, previousValue) => {
      if (chan && !chan.config.apiToken) {
        root.$set(chan.config, 'apiToken', '')
      }
