              ...
              <v-container class="pa-0">
                ...
                <v-row no-gutters v-if="filter.attr && (!filter.attr.endsWith('#status') || filter.attr === 'attr#status')">
                  <v-col cols="12">
                    ...
                    <RelationAttributeSearchComponent v-if="filter.operation !== 10 && filter.operation !== 19 && filter.operation !== 16 && filter.operation !== 17 && filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && getAttrType(filter) === AttributeType.Relation" :operation="filter.operation" :attrIdentifier="getAttrIdentifier(filter)" v-model="filter.value" />
                    ...
                    <v-textarea v-if="(filter.operation === 10 || filter.operation === 19) && filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && !lovsMapRef[filter.attr]" dense v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required></v-textarea>
                  </v-col>
                </v-row>
                <v-row no-gutters v-if="filter.attr && filter.attr.endsWith('#status') && filter.attr !== 'attr#status'">
                  <v-col cols="12">
      ...
      Promise.all([loadAllTypes(), loadAllLanguages(), loadAllAttributes(), loadAllChannels()]).then(async () => {
        ...
        for (let i = 0; i < attrs.length; i++) {
          ...
          if (attr.languageDependent) {
            ...
            arr.push(data)
            if (attr.lov && attr.type === AttributeType.LOV && !lovsMapRef.value[val]) lovsMapRef.value[val] = attr.lov
          }
        }
