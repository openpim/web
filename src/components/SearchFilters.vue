              ...
              <v-container class="pa-0">
                ...
                <v-row no-gutters v-if="filter.attr && (!filter.attr.endsWith('#status') || filter.attr === 'attr#status')">
                  <v-col cols="12">
                    ...
                    <RelationAttributeSearchComponent v-if="filter.operation !== 10 && filter.operation !== 16 && filter.operation !== 17 && filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && getAttrType(filter) === AttributeType.Relation" :operation="filter.operation" :attrIdentifier="getAttrIdentifier(filter)" v-model="filter.value" />
